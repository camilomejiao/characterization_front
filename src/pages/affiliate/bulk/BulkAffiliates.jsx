...
const validationSchema = Yup.object({
    ...
    attachment: Yup.mixed()
        ...
        .test(
            "fileFormat",
            "Solo se permiten archivos CSV (.csv)",
            (value) => !value || value.name.toLowerCase().endsWith(".csv"),
        )
...
const normalizeHeader = (value) =>
    (value || "").toString().trim().toUpperCase().replace(/\s+/g, " ").replace(/\t+/g, " ");
...
//
const IDENTIFICATION_TYPE = ["AS", "CC", "CD", "CE", "CN", "PA", "PE", "RC", "SC", "TI", "PT"];
...
export const BulkAffiliates = () => {
    ...
    const validateRow = (rowKnown, required, rowNumber) => {
        ...
        if (!isEmptyValue(rowKnown["TIPO_DOCUMENTO"])) {
            ...
            if (/^\d+$/.test(td)) {
                errors.push(
                    `Fila ${rowNumber}: TIPO_DOCUMENTO no debe ser numérico (usa códigos como CC, TI, CE, PA).`,
                );
            }

            if (!/^[A-Za-z]{1,5}$/.test(td)) {
                errors.push(
                    `Fila ${rowNumber}: TIPO_DOCUMENTO con formato inválido (solo letras, 2 caracteres).`,
                );
            }
        ...
        if (!isEmptyValue(rowKnown["SEXO"])) {
            ...
            if (/^\d+$/.test(td)) {
                errors.push(
                    `Fila ${rowNumber}: SEXO no debe ser numérico (usa códigos como F, M).`,
                );
            } else if (!/^[A-Za-z]{1,5}$/.test(td)) {
                errors.push(
                    `Fila ${rowNumber}: SEXO con formato inválido (solo una letra, 1 caracteres).`,
                );
            }
        }
        ...
        if (!isEmptyValue(rowKnown["ESTADO"])) {
            ...
            if (/^\d+$/.test(td)) {
                errors.push(
                    `Fila ${rowNumber}: ESTADO no debe ser numérico (usa códigos como AC, AF,RE).`,
                );
            }

            if (!/^[A-Za-z]{1,5}$/.test(td)) {
                errors.push(
                    `Fila ${rowNumber}: ESTADO con formato inválido (solo una letra, 2 caracteres).`,
                );
            }
        ...
        if (!isEmptyValue(rowKnown["FECHA_NACIMIENTO"])) {
            ...
            if (!iso)
                errors.push(
                    `Fila ${rowNumber}: FECHA_NACIMIENTO con formato inválido (esperado dd/mm/yyyy)`,
                );
        }
        ...
        if (!isEmptyValue(rowKnown["ZONA"])) {
            ...
            if (/^\d+$/.test(td)) {
                errors.push(
                    `Fila ${rowNumber}: ZONA no debe ser numérico. Códigos permitidos: ${AREA_TYPE.join(", ")}`,
                );
            }

            if (!/^[A-Za-z]{1,5}$/.test(td)) {
                errors.push(
                    `Fila ${rowNumber}: ZONA con formato inválido (solo una letra, 1 caracteres).`,
                );
            }
        ...
        if (!isEmptyValue(rowKnown["EPS"])) {
            ...
            if (!/^[A-Z0-9]{1,6}$/.test(epsTxt)) {
                errors.push(
                    `Fila ${rowNumber}: EPS con formato inválido (solo letras y números, máximo 6). Ej: EPS002`,
                );
            }
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                ...
                if (!expectedPrefix) {
                    AlertComponent.warning(
                        "No se pudo identificar el prefijo del régimen seleccionado.",
                    );
                    setLoading(false);
                    return;
                }
    ...
    const formatValidationError = (err) => {
        ...
        try {
            ...
            if (Array.isArray(parsed)) {
                finalDetail = parsed.map((x) => `${x.field}: ${x.errors.join(", ")}`).join(" | ");
            }
        } catch (e) {}
        ...
        if (Array.isArray(source?.pointer)) {
            pointer =
                " (" +
                source.pointer.map((p) => `${p.field}: ${p.errors.join(", ")}`).join(" | ") +
                ")";
        }
